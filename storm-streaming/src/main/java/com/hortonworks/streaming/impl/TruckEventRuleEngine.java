...
public class TruckEventRuleEngine implements Serializable {
	...
	public void initialize(){
		try{
            ...
            producer = getTopicProducer(session);
...
//			getTopicProducer(session).send(message);
			producer.send(message, getTopicProducer(session).getDeliveryMode(), getTopicProducer(session).getPriority(), ACTIVEMQ_MESSAGE_TTL);
		} catch (JMSException e) {
			LOG.error("Error sending TruckDriverViolationEvent to topic", e);
			return;
		}/*finally{
